...
// 1) API 엔드포인트 - 변수로 변경
// ================================
const SHEETBASE = API_CONFIG.CREW_API;  // ✅ 변수로 변경
const RESERVATIONAPI = API_CONFIG.RESERVE_API;  // ✅ 변수로 변경
...
let allCrewDataRaw = []; // API로 받은 전체 크루 데이터
let crewDataForPanel; // 패널에 보여줄 크루 리스트
let crewDataForMap; // 지도에 마커로 표시할 크루 리스트
let userPosition = null;
let userMarker = null;
let markerList = [];

// sessionStorage에서 로그인 정보 불러오기 (login.js와 동일 키 사용)
let USER_EMAIL = null;
...
/**
 * navigator.geolocation.getCurrentPosition을 Promise로 감싸서
...
async function initializeUserLocation() {
  try {
    // Promise 기반의 위치 정보 함수를 await로 호출
...
async function initMapWithUser() {
  // initMap(및 내부의 loadCrewData)이 완료되기를 기다림
  await initMap();
  addUserMarker(); // 마커 추가는 맵 초기화가 끝난 후 실행
}
...
async function initMap() {
  ...
  // loadCrewData가 완료될 때까지 기다림
  // 이 함수가 완료되어야 첫 "근처" 탭 렌더링이 보장됨
  await loadCrewData(); // 여기서 allCrewDataRaw, crewDataForPanel, crewDataForMap이 모두 채워짐

  // addUserMarker()는 initMapWithUser로 이동 (중복 제거)
  ...
  setTimeout(() => {
    document.querySelectorAll(".crew-marker").forEach((markerEl) => {
      markerEl.onclick = () => {
        ...
        // 'crewDataForPanel'(현재 패널 목록) 또는 'allCrewDataRaw' (전체)에서 크루 정보 찾기
...
function getCrewCardHTML(crew, userDistance, courseDistance) {
  // 이미지 URL 파싱 (콤마로 구분된 첫 번째 이미지만 사용)
  ...
  if (crewList.length === 0) {
    ...
    if (isMyReservationMode) {
      crewListPanel.innerHTML =
        '<p class="no-reservation">다가오는 예약이 없습니다.</p>'; // 메시지 변경
    } else {
      crewListPanel.innerHTML =
    ...
    oncomplete: function (data) {
      ...
      geocoder.addressSearch(fullAddr, function (result, status) {
        if (status === kakao.maps.services.Status.OK) {
          ...
          // 'allCrewDataRaw' (전체 크루) 기준으로 거리 계산
          ...
          // 3. 지도/패널 업데이트
          clearMarkers();
          addCrewMarkers(crewDataForMap); // 맵에는 '가장 빠른' 마커만 표시
          renderCrewCards(crewDataForPanel); // 패널에는 '전체' 목록 표시
...
async function loadCrewData() {
  try {
    const res = await fetch(`${API_CONFIG.CREW_API}/tabs/CREWLIST`);
...
async function showMyReservations(userEmail) {
  try {
    const res = await fetch(API_CONFIG.RESERVE_API)
;
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    showCustomToast('지도를 불러올 수 없습니다.', true);
  }
});
